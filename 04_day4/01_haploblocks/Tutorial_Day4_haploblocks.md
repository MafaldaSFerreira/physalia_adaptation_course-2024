...
IMPORTANT: Please copy the folder `04_day4` from `~/Share` into you repository on the AWS server. Please copy it also to your own computer and try to follow the same folder architecture when moving your files.
...
Look at the format. We have 3 columns for describing the coordinates of each window, total eigen values, eigvalue of PC1, eigvalue of of PC2, then 240 values for PC1 scores of all our samples, and 240 values for PC2 scores of all samples.
```R
...
We next want to know which windows explain the pattern observed in the global PCA where we first identified the three haplogroups. We can look at the correlation between the PCs of the global PCA analysis and PC1 of each local PCA. I propose to use the PCA performed on the 12 NWA populations, and look at the correlation between PC1 of the global PCA and PC1 of each local PCA (then you can do the same with PC2 of the global PCA and PC1 of each local PCA...). To help, the geno-transformed matrix is inside your folder so that you don't have to generate it with VCFtools but the `.012` file has been done exactly as shown on day 2.

Bellow, you can find a very basic loop to store the correlation by windows. You can probably do something more fancy :-)
Please note here that we could also have used the genotype correlation, for instance call 0/1 or 0/1/2 the cluster observed on the global pca and then look for each SNP at the correlation between genotypes and genotype for the cluster identified.
...
```
Let's now merge the correlation and position tables to do a Manhattan plot of correlations along the genome. We need to take a midposition for each window in the local PCA.
...
As you see, MDS 1 and 2 are largely driven by Chr4 and Chr5. Let's look at the MDS scores along the genome to pinpoint the regions within chromosomes that are driving the signal.
Building on what we did before, you can generate a Manhattan plot with a midposition as XX, and MDS1 or MDS2 as YY.
...
We have located the breakpoints approximately at 4.8MB to 16.6MB. We can make a PCA based on variants in that region only and use k-means approaches to classify the individuals into the three possible genotypes for the inversion (homozygotes standard AA, heterozygotes AB, homozygotes inversion BB). 
...
If you are interested in understanding how to perform the genotyping, you can have a look in this file
...
PLINK requires three inputs (`.bed`, `.bim`, `.fam`). The argument `--r2` calculates LD as an R<sup>2</sup> correlation between the genotypes at pair of SNPs, `--inter-chr` makes a long matrix (`--square` would make a square one).
...
We want to consider the same SNPs so we used the recoded VCF with MAF >5% anove, but just keep the AA individuals using `--keep` in VCFtools.
...
As you can note, within our region of interest on Chr4, some SNPs have very high F<sub>ST</sub> (up to 1), suggesting fixed alleles and extremely high differentiation.
You may also have noticed that some F<sub>ST</sub> values are negative. This is likely driven by very low frequency alleles and inbalanced sample sizes. We also observe NA in the calculation of F<sub>ST</sub> by site.
...
### Studying heterozygosity with the % of heterozygotes in each genotype group (optional)
