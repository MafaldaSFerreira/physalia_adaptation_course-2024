# Functional annotation of candidate loci for adaptation <!-- omit from toc -->
...
- [Step 1. SNP annotation with SNPeff](#step-1-snp-annotation-with-snpeff)
    - [Annotate the VCF file](#annotate-the-vcf-file)
- [Step 2 Bedtools : find the intersection between SNPs and genes](#step-2-bedtools--find-the-intersection-between-snps-and-genes)
...
- [Step3 Goseq Gene ontology enrichment analysis](#step3-goseq-gene-ontology-enrichment-analysis)
    - [Prepare our correspondance transcript/GO](#prepare-our-correspondance-transcriptgo)
...
You don't need to copy them as we have prepared simplified files (in R, simply selecting the relevant column) when needed. You may want to have a look to get a sense of what it looks like using `less ~/Share/ressources/genome_mallotus_dummy.gff3`. Press `q` to exit the less visualisation. 
...
Here, we have already built the database for you as this is a long process and takes some space on the server. If you want to replicate this process, you can see the pipeline here [Make SNPeff DB](SNPeff_createDB.md) (but do not try to run it on the server please).
If you want to re-creat it, the `.gff` is inside the `02_data` while the reference genome is in the folder `01_day1/02_genome`.
...
## Step 3. Gene ontology enrichment analysis with goseq
...
Please copy *the whole folder `05_day5`* to your local computer, and set your working directory in Rstudio to `05_day5`.
...
#### Prepare a file with the correspondance between transcrips and gene ontology terms
...
Please install the library splitstackshape which is able to split a whole column, and the library data.table which is super helpful to transform wide table into a long table.
```R
...
Yeah! We have formatted all the files. Let's now test enrichment using our database (transcript/GO) named "go_ready" and the prepared list of genes with 0/1 info for our outliers from temperature association with RDA.
